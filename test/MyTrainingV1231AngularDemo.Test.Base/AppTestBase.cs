...
using MyTrainingV1231AngularDemo.Test.Base.TestData;
...
namespace MyTrainingV1231AngularDemo.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="MyTrainingV1231AngularDemoDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(MyTrainingV1231AngularDemoDbContext context)
            {
        ...
        protected void UsingDbContext(Action<MyTrainingV1231AngularDemoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MyTrainingV1231AngularDemoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<MyTrainingV1231AngularDemoDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<MyTrainingV1231AngularDemoDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MyTrainingV1231AngularDemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyTrainingV1231AngularDemoDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MyTrainingV1231AngularDemoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyTrainingV1231AngularDemoDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<MyTrainingV1231AngularDemoDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<MyTrainingV1231AngularDemoDbContext, Task<TResult>> func)
        {
            TResult result;
